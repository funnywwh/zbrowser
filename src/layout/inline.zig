...
pub fn layoutInline(layout_box: *box.LayoutBox, containing_block: box.Size) !*context.InlineFormattingContext {
    // 检查是否已有IFC，如果存在则先清理
    if (layout_box.formatting_context) |existing_ctx| {
        // 假设existing_ctx指向的是InlineFormattingContext（因为这是inline布局函数）
        // 这是一个unsafe操作，但在inline布局的上下文中是安全的
        const existing_ifc: *context.InlineFormattingContext = @ptrCast(@alignCast(existing_ctx));
        deinitIFC(existing_ifc, layout_box.allocator);
    ...
    var current_line = try createLineBox(ifc, current_y, layout_box.allocator);
    ...
    for (layout_box.children.items) |child| {
        ...
        if (line_width + child_width > available_width and line_width > 0) {
            ...
            current_line = try createLineBox(ifc, current_y, layout_box.allocator);
            line_width = 0;
            line_height = 0;
        }
