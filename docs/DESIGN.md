...
6. **字体模块**：字体加载、解析和字形渲染（已集成到渲染后端）
7. **图像输出模块**：将画布编码为PNG

#### 扩展模块
8. **JavaScript引擎模块**：解析和执行JavaScript
9. **DOM API模块**：提供DOM操作接口
10. **事件模块**：处理用户事件和DOM事件
11. **动画模块**：处理CSS动画和过渡
...
├── ttf.zig           # TTF/OTF字体解析器（TtfParser）
└── glyph.zig         # 字形渲染器（GlyphRenderer）
```
