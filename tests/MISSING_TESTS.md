...
2. ~~**Parser.parse 边界情况**~~ ✅ 部分添加
   - ✅ 测试空HTML字符串
   - ✅ 测试只有空白字符的HTML
   - ❌ **待添加：测试不完整的HTML标签**（如 `<div` 没有闭合，应该能容错处理）
   - ❌ **待添加：测试嵌套错误的HTML**（如未闭合的标签 `<div><p></div>`，应该能容错处理）
   - ❌ **待添加：测试特殊字符和实体编码**（如 `&lt;`, `&gt;`, `&amp;`, `&quot;`, `&#39;` 等）
   - ❌ **待添加：测试Unicode字符**（如中文、emoji等）
...
2. ~~**Tokenizer.next 边界情况**~~ ✅ 部分添加
   - ✅ 测试不完整的标签（如 `<div` 没有闭合）
   - ✅ 测试不完整的属性（如 `class="test` 没有闭合引号）
   - ✅ 测试不完整的注释（如 `<!-- test` 没有闭合）
   - ❌ **待添加：测试不完整的CDATA**（如 `<![CDATA[content` 没有闭合）
   - ❌ **待添加：测试不完整的DOCTYPE**（如 `<!DOCTYPE html` 没有闭合）
   - ❌ **待添加：测试特殊字符**（如 `<`, `>`, `&`, `"`, `'` 在属性值中）
   - ❌ **待添加：测试Unicode字符**（如中文标签名、属性值、文本内容）
   - ❌ **待添加：测试emoji字符**（在文本和属性值中）
...
4. ~~Parser.parse 边界情况~~ ✅ 部分完成（空HTML、空白HTML已测试）
5. ~~Tokenizer.next 边界情况~~ ✅ 部分完成（不完整标签、属性、注释已测试）
